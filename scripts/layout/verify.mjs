...
for(const scene of imported.grid.snapshots){
 ...
 for(const col of cols){
  ...
  assert.equal(content.find(n=>n.type==='rectangle')?.width,col.width-(col.inputs.gutter||0),'Gutter remains inside Col');
  importedCols++;
 }
...
const serializedMasonryChild=JSON.stringify({type:'rectangle',name:'Serialized item',fill:'#1677FF'}),serializedMasonry=render('Masonry',{columns:'2',gutter:'16',items:JSON.stringify([{key:1,height:80,children:serializedMasonryChild}])},360,120);assert.equal(serializedMasonry[0].type,'rectangle');assert.equal(serializedMasonry[0].width,172);assert.equal(serializedMasonry[0].height,80);
assert.equal(render('Card.content',{children:'1'},172,80).find(node=>node.type==='frame')?.children?.[0]?.content,'1','Card text children must render as text instead of an unresolved component reference');
const serializedCardChild=JSON.stringify({type:'frame',name:'Card Meta',children:[{type:'text',name:'Title',content:"I'm Special"}]}),serializedCardCover=JSON.stringify({type:'rectangle',name:'Food image',height:120,fill:'#DDD'}),serializedCard=render('Card.content',{children:serializedCardChild,cover:serializedCardCover,size:'small'},172,196);assert.equal(serializedCard.find(node=>node.name==='Food image')?.height,120,'Card cover must render as a canvas node');assert.equal(serializedCard.find(node=>node.name==='Card body')?.children?.[0]?.name,'Card Meta','Card children must render as a canvas node instead of JSON text');
for(const scene of imported.masonry.snapshots){
 const expectedCount=JSON.parse(scene.props.items).length;
...
const wrapProps={wrap:true,gap:'small',children:JSON.stringify(Array.from({length:24},()=>({ref:'CjMCs',width:78,height:32})))};
...
const verticalDefault=render('Space',{vertical:true,children:JSON.stringify([{ref:'CjMCs',width:78,height:32},{ref:'CjMCs',width:120,height:32}])},200,80);
assert.deepEqual(verticalDefault.map(node=>node.x),[0,0],'Vertical Space defaults to start alignment');
const spaceSource=fs.readFileSync(new URL('../../canvas-components/Space.js',import.meta.url),'utf8');
assert(spaceSource.includes('@input separator: ref'),'Space separator must be exposed as a ReactNode reference');
assert(!spaceSource.includes('align: enum("start", "center", "end", "baseline") = "center"'),'Space align must not expose a non-official center default');
const compactWidths=render('Space.Compact',{block:true,children:JSON.stringify([{ref:registry.components.Button,width:120,height:32},{ref:registry.components.Button,width:80,height:32}])},400,32);
assert.deepEqual(compactWidths.map(node=>node.width),[120,80],'Space.Compact block preserves explicit child widths');
const longRow={gutter:'[16,16]',children:JSON.stringify(Array.from({length:8},(_,order)=>({ref:registry.components.Col,children:'Column',background:'#0092ff',span:6,height:120,order})))};
...
const collapsePanels=[0,1,2].map(index=>({ref:registry.components['Splitter.Panel'],size:'33.333%',collapsible:{start:true,end:true,showCollapsibleIcon:true}}));
const equalPanels=render('Splitter',{children:JSON.stringify(collapsePanels.map(({size,...panel})=>panel))},740,200).filter(node=>node.name.startsWith('children['));
assert(equalPanels.every(node=>Math.abs(node.width-740/3)<0.001),'Panels without size use the official equal-width default');
const visibleCollapse=render('Splitter',{children:JSON.stringify(collapsePanels)},740,200);
assert.equal(visibleCollapse.filter(node=>node.name==='collapse start').length,2,'Every Splitter bar exposes its start collapse control');
assert.equal(visibleCollapse.filter(node=>node.name==='collapse end').length,2,'Every Splitter bar exposes its end collapse control');
assert(visibleCollapse.findIndex(node=>node.name==='collapse end')>visibleCollapse.findLastIndex(node=>node.name.startsWith('children[')),'Collapse controls render above every panel');
const firstCollapseSurface=visibleCollapse.find(node=>node.name==='collapse start surface'&&node.x<300);
assert(Math.abs(firstCollapseSurface.x-231.6667)<0.001);
assert.deepEqual({...firstCollapseSurface,x:0},{type:'rectangle',name:'collapse start surface',x:0,y:88,width:12,height:24,fill:'#0000000F',cornerRadius:2});
const automaticCollapse=render('Splitter',{children:JSON.stringify(collapsePanels.map(panel=>({...panel,collapsible:true})))},740,200);
assert.equal(automaticCollapse.filter(node=>node.name.startsWith('collapse ')).length,0,'Auto collapse controls stay hidden until the divider is hovered');
const disabledBar=render('Splitter',{children:JSON.stringify(collapsePanels.slice(0,2).map((panel,index)=>({...panel,resizable:index!==0})))},740,200);
assert.equal(disabledBar.filter(node=>node.name==='split bar').length,1,'A disabled Splitter keeps the visual divider');
assert.equal(disabledBar.filter(node=>node.name==='dragger').length,0,'A disabled Splitter hides the draggable marker');
