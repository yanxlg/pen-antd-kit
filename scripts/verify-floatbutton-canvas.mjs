...
function renderNode({node,badge}) {
 ...
 const output=layers.flatMap(n=>{if(n.type!=='script')return n;const name=n.scriptUri.split('/').pop().replace(/\.js$/,'');return renderIcon({input:{...n.inputs,name},width:n.width,height:n.height}).map(p=>({...p,x:p.x+n.x,y:p.y+n.y}));});
 if(badge){
...
assert.equal(renderFloatButton({input:fixture({}).node.inputs}).find(n=>n.type==='script').scriptUri,'../canvas-components/icons/FileTextOutlined.js');
assert.ok(!Object.hasOwn(renderFloatButton({input:fixture({}).node.inputs}).find(n=>n.type==='script').inputs,'name'));
