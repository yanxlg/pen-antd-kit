...
const verticalCompact=renderButton({input:{children:'Button 1',type:'primary',compactPlacement:'start',compactOrientation:'vertical'},width:120,height:32});
const verticalDivider=verticalCompact.find((node,index)=>index>0&&node.type==='rectangle'&&node.fill==='#4096FF');
assert.deepEqual({x:verticalDivider.x,y:verticalDivider.y,width:verticalDivider.width,height:verticalDivider.height},{x:0,y:31,width:120,height:1});
assert.ok(!verticalCompact.some((node,index)=>index>0&&node.type==='rectangle'&&node.x===119&&node.width===1&&node.height===32));
