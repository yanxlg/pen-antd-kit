...
for(const f of fs.readdirSync('canvas-components').filter(f=>f.endsWith('.js'))){const d={};for(const m of fs.readFileSync('canvas-components/'+f,'utf8').matchAll(/@input (\w+):[^\n]*? = ([^\n]+)/g)){try{d[m[1]]=JSON.parse(m[2])}catch{d[m[1]]=m[2]}}defaults[f.slice(0,-3)]=d;}
export function operation(name){const needed=new Set([name,...plan[name].flatMap(g=>g.samples.map(s=>s.component||name)),'Button','Avatar']);if(needed.has('Card'))needed.add('Card.content');const selectedDefaults=Object.fromEntries([...needed].filter(k=>defaults[k]).map(k=>[k,defaults[k]]));return `const name=${JSON.stringify(name)},groups=${JSON.stringify(plan[name])},defaults=${JSON.stringify(selectedDefaults)};
...
if(process.argv[1]?.endsWith('/build.mjs')){fs.mkdirSync('scripts/component-showcase/generated',{recursive:true});for(const name of Object.keys(plan))fs.writeFileSync(`scripts/component-showcase/generated/${name}.pencil.js`,operation(name));console.log(Object.keys(plan).length+' Components plans generated');}
