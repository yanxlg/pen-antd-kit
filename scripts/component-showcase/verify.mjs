...
const tagSource=fs.readFileSync('canvas-components/Tag.js','utf8');for(const color of ['default','success','processing','warning','error','purple']){const nodes=new Function('pencil',tagSource)({input:{...defaults.Tag,color,variant:'solid',closable:true},width:120,height:22});const label=nodes.find(n=>n.name==='children'),surface=nodes.find(n=>n.name==='Tag surface');assert.notEqual(label.fill,surface.fill,'Solid tag label must remain visible');assert.equal(label.fill,'#FFFFFF');assert.equal(nodes.find(n=>n.type==='ref').inputs.color,'#FFFFFF');}
