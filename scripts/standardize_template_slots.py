...
if gn:
    ...
    updates_plan.append(("Gn0rZ", {
        "name": gn["name"],
        "slot": [],
        "context": gn["context"]
    }))

# xfNTc (Breadcrumb Slot)
xf = find_node(lib_data, "xfNTc")
if xf:
    xf["name"] = "Breadcrumb Slot · 面包屑插槽"
    xf["slot"] = []
    updates_plan.append(("xfNTc", {
        "name": xf["name"],
        "slot": []
    }))

# Db9uo (App Switcher Slot)
db = find_node(lib_data, "Db9uo")
if db:
    db["name"] = "App Switcher Slot · 切换器插槽"
    db["slot"] = []
    updates_plan.append(("Db9uo", {
        "name": db["name"],
        "slot": []
    }))
...
try:
    mcp = PenMCP()
    js_code = []
    for nid, props in updates_plan:
        js_code.append(f'Update("{nid}", {json.dumps(props, ensure_ascii=False)});')
    
    # 额外更新 N0FHv 子文字
    js_code.append('Update("yChbj", {name: "插槽标题", content: "页面业务内容插槽 (Page Body Slot)"});')
    js_code.append('Update("BKUi8", {name: "插槽说明", content: "在实例中直接替换此 Slot 填充业务页面；外层固定保留 16px 内容间距"});')
    
    # 打印确认
    js_code.append('Print("Live Pencil update complete! Total updated: " + ' + str(len(updates_plan)) + ');')
    
    full_js = "\n".join(js_code)
    res = mcp.execute(full_js)
    print("Pencil live execution result:")
    print(res)
    mcp.close()
except Exception as e:
    print("Warning: Live sync encountered error (socket might be offline):", e)
