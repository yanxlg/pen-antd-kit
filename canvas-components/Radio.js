...
if(i.optionType==='button'){
 ...
 nodes.push({type:'rectangle',x:0,y:0,width:pencil.width,height:pencil.height,fill:i.disabled?'#F5F5F5':selected&&solid?primary:'#fff',stroke:i.disabled?'#D9D9D9':selected?primary:'#d9d9d9',strokeWidth:1});
 ...
 return nodes;
  ...
  nodes.push({type:"ellipse", x:0, y:(h-s)/2, width:s, height:s, fill:i.disabled?"#F5F5F5":"#FFFFFF", stroke:i.disabled?"#D9D9D9":(isChecked?primary:"#D9D9D9"), strokeWidth:1});
  if (isChecked) nodes.push({type:"ellipse", x:4, y:(h-s)/2+4, width:8, height:8, fill:i.disabled?"#BFBFBF":primary});
