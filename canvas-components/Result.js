...
const numeric=['403','404','500'].includes(i.status),kinds={success:['CheckCircleFilled','#52C41A'],info:['InfoCircleFilled','#1677FF'],warning:['WarningFilled','#FAAD14'],error:['CloseCircleFilled','#FF4D4F']},c=kinds[i.status]||kinds.info,top=48,imgH=numeric?295:72;
if(numeric)nodes.push({type:'rectangle',name:'Official result illustration',x:(W-250)/2,y:top,width:250,height:295,fill:{type:'image',url:'/Users/yanxianliang/overseas/pen-antd-kit/libraries/images/antd-result-'+i.status+'.png',mode:'fit'}});else nodes.push(icon(c[0],(W-72)/2,top,72,c[1]));let y=top+imgH+24;nodes.push(text(i.title,24,y,W-48,32,24,'#000000E0','400','center'));y+=40;nodes.push({...text(i.subTitle,32,y,W-64,44,14,'#00000073','400','center'),textAlignVertical:'top'});y+=52;if(i.extra)child(i.extra,24,y,W-48,40);if(i.children)child(i.children,40,y+64,W-80,Math.max(40,H-y-88));
