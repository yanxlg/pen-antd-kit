 ...
 * @input listItemScrollOffset: number = 0
 ...
 * @input showCheckedStrategy: enum("SHOW_PARENT", "SHOW_CHILD", "SHOW_ALL") = "SHOW_PARENT"
 * @input transitionName: string = ""
 * @input treeCheckStrictly: boolean = false
 * @input treeDefaultExpandAll: boolean = false
 * @input treeNodeFilterProp: string = ""
 * @input treeNodeLabelProp: string = ""
...
const iconPath=(geometry,viewBox,x,y,size=14,color="#00000040")=>({type:"path",x,y,width:size,height:size,viewBox,geometry,fill:color});
  ...
  if (i.multiple) {
    ...
    if (label) {
      ...
      if (placeholder) nodes.push(text(placeholder, pad, (h-18)/2, W - 32 - pad, "#00000040", 14));
    }
    ...
    if (val) nodes.push(text(val, pad, (h-18)/2, W - rightPad - pad, i.value ? disabled : "#00000040", 14));
    if (effFeedback !== "none") {
