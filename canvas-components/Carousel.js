...
const slides=String(i.children||'1|2|3|4').split('|'),active=Math.min(slides.length-1,Math.max(0,Number(i.initialSlide)||0));nodes.push(box(0,0,W,H,'#364D79'));nodes.push(text(slides[active],32,(H-28)/2,W-64,28,16,'#FFFFFF','500','center'));
if(i.arrows){nodes.push(text('‹',8,(H-32)/2,24,32,28,'#FFFFFF','400','center'));nodes.push(text('›',W-32,(H-32)/2,24,32,28,'#FFFFFF','400','center'));}
if(i.dots!==false){const vertical=['start','end'].includes(i.dotPlacement),length=slides.length*24+8;let cursor=(vertical?H:W)/2-length/2;slides.forEach((_,n)=>{const len=n===active?24:16;nodes.push(box(vertical?(i.dotPlacement==='start'?12:W-15):cursor,vertical?cursor:(i.dotPlacement==='top'?12:H-15),vertical?3:len,vertical?len:3,n===active?'#FFFFFF':'#FFFFFF4D',2));cursor+=len+8;});}
